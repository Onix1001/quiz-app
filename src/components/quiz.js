...
import { over } from '../features/quizSlice'
...
const Quiz = () => {
    ...
    const kirimJwbn = (e) => {
        ...
        radio.filter(e => e.checked).map(e => {
            if (e.value === 'true') {
    ...
    return (
        <>
            <div className="min-h-screen font-comic p-5 bg-teriary text-center flex flex-col gap-y-10 items-center">
                <form onSubmit={kirimJwbn.bind(this)} className="flex flex-col items-start text-left gap-y-10 bg-white w-full p-5 rounded-md shadow-lg lg:w-2/5">
                    {qa.map((e, i) => {
                        return (
                                ...
                                <div>{e.jawaban.map((el, i) => {
                                    return (
                                        <div key={i} className="border py-3 px-2 w-full flex justify-start items-center gap-x-2 mt-2 cursor-pointer">
                                            <input className="accent-primary" id={el.jawab} type="radio" value={el.isBetul} name={e.pertanyaan} required />
